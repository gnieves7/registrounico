...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Find sessions that are approximately 12 hours away
    ...
    for (const session of sessions as Session[]) {
      ...
      if (profileError || !profile?.email) {
        ...
        emailResults.push({
          ...
          success: false,
          error: "No email found for patient"
        });
        continue;
      }
      ...
      try {
        const emailResponse = await resend.emails.send({
          from: "Recordatorios <noreply@tudominio.com>", // Cambia esto por tu dominio verificado
          ...
          html: `
            ...
            <body style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f5f5f5;">
              <div style="background-color: #ffffff; border-radius: 12px; padding: 32px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                ...
                <p style="color: #333; font-size: 16px; line-height: 1.6;">
                  Te recordamos que tienes una sesión programada para mañana:
                </p>
                ...
                <p style="color: #333; font-size: 16px; line-height: 1.6;">
                  Te recomendamos preparar tus preguntas o temas que quieras tratar durante la sesión.
                </p>
                
                <p style="color: #666; font-size: 14px; line-height: 1.6; margin-top: 32px;">
                  Si necesitas reprogramar tu cita, por favor contáctanos con anticipación.
                </p>
