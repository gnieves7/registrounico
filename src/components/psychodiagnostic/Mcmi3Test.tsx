...
const TOTAL_QUESTIONS = 175;

export function Mcmi3Test({ existingTest, onComplete }: Mcmi3TestProps) {
  const { user } = useAuth();
  const { toast } = useToast();
  const [currentPage, setCurrentPage] = useState(0);
  const [responses, setResponses] = useState<Record<number, "V" | "F">>({});
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (existingTest?.responses && Array.isArray(existingTest.responses)) {
      const map: Record<number, "V" | "F"> = {};
  ...
  const currentQuestions = mcmi3Questions.slice(startIdx, endIdx);
  const answeredCount = Object.keys(responses).length;
  const progress = (answeredCount / TOTAL_QUESTIONS) * 100;

  const handleAnswer = (questionNum: number, answer: "V" | "F") => {
    setResponses((prev) => ({ ...prev, [questionNum]: answer }));
  };
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            <CardTitle>MCMI-III — Inventario Clínico Multiaxial de Millon</CardTitle>
            <CardDescription className="mt-1">
              Lea cada frase y marque Verdadero (V) si describe su forma de ser, o Falso (F) si no la refleja.
            </CardDescription>
          </div>
            ...
            >
              ...
              <div className="flex gap-2 shrink-0">
                <Button
                  ...
                  className="h-8 w-12 transition-all duration-200 hover:scale-105 active:scale-95"
                  onClick={() => handleAnswer(questionNum, "V")}
                >
                  V
                </Button>
                <Button
                  ...
                  className="h-8 w-12 transition-all duration-200 hover:scale-105 active:scale-95"
                  onClick={() => handleAnswer(questionNum, "F")}
                >
                  F
                </Button>
              </div>
