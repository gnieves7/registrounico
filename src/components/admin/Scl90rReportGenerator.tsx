...
<html><head><meta charset="utf-8"><title>Informe SCL-90-R — ${h(patientName || 'Paciente')}</title>
<style>
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      await supabase.from("app_notifications").insert({
        ...
        message: priceNum > 0
          ? `Tu informe SCL-90-R está listo. Precio: $${priceNum}. Realizá el pago para obtener el código de descarga.`
          : "Tu informe SCL-90-R está listo y disponible para descargar.",
        related_table: "documents",
        related_record_id: docData?.id || null,
        route: "/documents",
        metadata: { test_type: "SCL-90-R", price: priceNum },
      });
