...
const ProfessionalProfile = () => {
  ...
  // --- Education CRUD ---
  const openAddEdu = () => {
    setEditingEdu({ title: "", institution: "", subtitle: "", item_type: "education" });
    setEduDialogOpen(true);
  };
  const openEditEdu = (item: ProfileItem) => {
    setEditingEdu({ ...item });
    setEduDialogOpen(true);
  };
  const saveEdu = async () => {
    if (!editingEdu?.title || !editingEdu?.institution || !editingEdu?.subtitle) {
      toast.error("Completá todos los campos"); return;
    }
    if (editingEdu.id) {
      const { error } = await supabase.from("professional_profile_items").update({
        title: editingEdu.title, institution: editingEdu.institution, subtitle: editingEdu.subtitle,
      }).eq("id", editingEdu.id);
      if (error) { toast.error("Error al actualizar"); return; }
      toast.success("Formación actualizada");
    } else {
      const { error } = await supabase.from("professional_profile_items").insert({
        item_type: "education", title: editingEdu.title, institution: editingEdu.institution,
        subtitle: editingEdu.subtitle, sort_order: education.length + 1,
      });
  ...
  // --- Service CRUD ---
  const openAddSvc = () => {
    setEditingSvc({ title: "", subtitle: "", description: "", icon_name: "Briefcase", features: [], is_popular: false, item_type: "service" });
    setSvcDialogOpen(true);
  };
  const openEditSvc = (item: ProfileItem) => {
    setEditingSvc({ ...item });
    setSvcDialogOpen(true);
  };
  const saveSvc = async () => {
    if (!editingSvc?.title || !editingSvc?.description) {
      toast.error("Completá título y descripción"); return;
    }
    const payload = {
      title: editingSvc.title, subtitle: editingSvc.subtitle || null,
      description: editingSvc.description, icon_name: editingSvc.icon_name || "Briefcase",
      features: editingSvc.features || [], is_popular: editingSvc.is_popular || false,
    };
    if (editingSvc.id) {
      const { error } = await supabase.from("professional_profile_items").update(payload).eq("id", editingSvc.id);
      if (error) { toast.error("Error al actualizar"); return; }
      toast.success("Servicio actualizado");
    } else {
      const { error } = await supabase.from("professional_profile_items").insert({
        ...payload, item_type: "service", sort_order: services.length + 1,
      });
      if (error) { toast.error("Error al agregar"); return; }
    ...
    <div className="space-y-8 pb-8 p-4 md:p-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4 text-sm text-muted-foreground leading-relaxed">
          <p><strong className="text-foreground">Licenciado en Psicología</strong> egresado de la Universidad Católica de Santa Fe y <strong className="text-foreground">Especialista en Psicología Forense</strong> por la Universidad Nacional de Rosario.</p>
          <p>Su práctica profesional se centra en la evaluación psicológica forense con sentido crítico y perspectiva evolutiva, aplicando técnicas especializadas de psicodiagnóstico en contextos judiciales civiles, penales y de familia.</p>
          <p>Cuenta con formación complementaria en criminología, pericias judiciales y protocolos especializados para la evaluación de víctimas de abuso sexual y violencia de género.</p>
        </CardContent>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Star className="h-5 w-5 text-primary" /> Áreas de Expertise</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2 text-lg"><Award className="h-5 w-5 text-primary" /> Afiliaciones Profesionales</CardTitle></CardHeader>
          <CardContent>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            {isAdmin && (
              <Button variant="outline" size="sm" onClick={openAddEdu} className="gap-1.5"><Plus className="h-4 w-4" /> Agregar</Button>
            )}
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {education.map((item) => (
                <div key={item.id} className="rounded-lg border bg-muted/30 p-4 space-y-2 relative group">
                  {isAdmin && (
                    <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => deleteEdu(item.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  )}
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h2 className="text-xl font-bold text-foreground flex items-center gap-2">
              <Briefcase className="h-5 w-5 text-primary" /> Servicios Profesionales
            </h2>
          ...
          {isAdmin && (
            <Button variant="outline" size="sm" onClick={openAddSvc} className="gap-1.5"><Plus className="h-4 w-4" /> Agregar</Button>
          )}
        </div>
        {loading ? (
          <p className="text-sm text-muted-foreground">Cargando...</p>
                ...
                <Card key={service.id} className="relative overflow-hidden group">
                  ...
                  {isAdmin && (
                    <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity" style={service.is_popular ? { top: "2.5rem" } : {}}>
                      ...
                      <Button variant="ghost" size="icon" className="h-7 w-7 text-destructive" onClick={() => deleteSvc(service.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                    </div>
                  )}
      ...
      <Dialog open={eduDialogOpen} onOpenChange={setEduDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={saveEdu}>{editingEdu?.id ? "Guardar" : "Agregar"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={svcDialogOpen} onOpenChange={setSvcDialogOpen}>
        <DialogContent className="max-w-lg">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    <Input value={featureInput} onChange={(e) => setFeatureInput(e.target.value)} placeholder="Nueva característica..." onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addFeature())} />
          ...
          <DialogFooter>
            ...
            <Button onClick={saveSvc}>{editingSvc?.id ? "Guardar" : "Agregar"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
